...
export default function Dashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      setActivity(actRes.data.bookings || [])
    } catch {
      // Use mock data if API not connected
      setStats({ total: 40, available: 24, occupied: 12, reserved: 4,
        zones: [
      ...
      setActivity([
        { _id: 1, vehicleNumber: 'RJ14 AB 1234', status: 'active',   slotId: 'B-07', createdAt: new Date().toISOString() },
        { _id: 2, vehicleNumber: 'MH12 EF 9012', status: 'reserved', slotId: 'C-14', createdAt: new Date().toISOString() },
        { _id: 3, vehicleNumber: 'DL01 XY 7890', status: 'active',   slotId: 'A-11', createdAt: new Date().toISOString() },
      ])
    } finally {
      setLoading(false)
    }
  ...
  useSocket({
    'slot:updated': () => fetchStats(),
    'booking:new': () => fetchStats(),
  })
      ...
      <div className={styles.chartsRow}>
        ...
        <div className={styles.panel}>
          <div className={styles.panelHeader}>
            <span>Hourly Traffic Today</span>
          </div>
