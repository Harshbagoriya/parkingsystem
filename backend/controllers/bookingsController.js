...
const CATEGORY_ZONE = {
  ...
  faculty:      'Faculty',const Booking     = require('../models/Booking')
const ParkingSlot = require('../models/ParkingSlot')
...
exports.adminReserve = async (req, res) => {
  try {
    ...
    const booking = await Booking.create({
      ...
      expiresAt, status: 'reserved',
      // Store holder details in booking notes field (attach to schema or use a flexible field)
      adminNotes: `Admin reserved by ${req.user.name}. Holder: ${holderName}${holderEmail ? ` (${holderEmail})` : ''}${holderMobile ? ` | Mobile: ${holderMobile}` : ''}${holderRole ? ` | Role: ${holderRole}` : ''}${notes ? ` | Notes: ${notes}` : ''}`,
    })
...
// GET /api/bookings/admin-reserve — admin get all reservations with holder details
